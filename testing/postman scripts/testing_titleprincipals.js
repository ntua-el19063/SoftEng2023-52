...
// Test 7: Additional test - Check if tconst, ordering, and nconst are valid values
pm.test("tconst, ordering, and nconst are valid values", function () {
    const responseData = pm.response.json();
    ...
    if (responseData.status === 'OK') {
        ...
        pm.expect(responseData).to.have.property('ordering').that.is.a('string');
        pm.expect(responseData).to.have.property('nconst').that.is.a('string');
    }
});

// Test 8: Additional test - Check if category is a valid string
pm.test("category is a valid string", function () {
    const responseData = pm.response.json();

    pm.expect(responseData).to.be.an('object');
    if (responseData.status === 'OK') {
        pm.expect(responseData.message).to.equal('File loaded successfully');
        pm.expect(responseData).to.have.property('category').that.is.a('string');
    }
});

// Test 9: Additional test - Check if imageURL is a valid string
pm.test("imageURL is a valid string", function () {
    const responseData = pm.response.json();

    pm.expect(responseData).to.be.an('object');
    if (responseData.status === 'OK') {
        pm.expect(responseData.message).to.equal('File loaded successfully');
        pm.expect(responseData).to.have.property('imageURL').that.is.a('string');
    }
});

// Test 10: Additional test - Check for error response when file structure is invalid
